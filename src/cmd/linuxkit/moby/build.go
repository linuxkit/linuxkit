...
func Build(m Moby, w io.Writer, pull bool, tp string, decompressKernel bool) error {
	...
	if m.Kernel.ref != nil {
		...
		err := ImageTar(m.Kernel.ref, m.Prefix, kf, enforceContentTrust(m.Kernel.ref.String(), &m.Trust), pull, "")
		if err != nil {
	...
	for _, ii := range m.initRefs {
		...
		err := ImageTar(ii, m.Prefix, iw, enforceContentTrust(ii.String(), &m.Trust), pull, resolvconfSymlink)
		if err != nil {
